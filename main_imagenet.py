...
parser.add_argument('--epochs', default='30,60,80,90', type=str, metavar='N',
...
def train(train_loader, model, criterion, optimizer, epoch, train_logger, args):
    ...
    train_logger.log({'epoch': epoch, 'loss': losses.avg, 'acc': float(top1.avg), 'acc5': float(top5.avg)})

def validate(epoch, val_loader, model, criterion, test_logger, args):
    batch_time = AverageMeter('Time', ':6.3f')
    losses = AverageMeter('Loss', ':.4e')
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    ...
    if test_logger is not None:
        test_logger.log({'epoch': epoch, 'loss': losses.avg, 'acc': float(top1.avg), 'acc5': float(top5.avg)})
    return top1.avg
...
def adjust_learning_rate(optimizer, epoch, args):
    import bisect
    index = bisect.bisect_right(args.epochs, epoch)
    lr_now = args.lr / (10 ** index)
    gamma_row = args.gamma / (10 ** index)
